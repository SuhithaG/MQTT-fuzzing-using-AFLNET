...
region_t* extract_requests_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  mem=(char *)ck_alloc(mem_size);
printf("buf size in rtsp request %d:\n", buf_size);
  ...
  while (byte_count < buf_size) {
	...
	//printf("cur end %d: \n", cur_end);
      //Check if the last byte has been reached
      if (cur_end == buf_size - 1) {
	printf("reached EOF, cur end %d %d: \n",cur_end, buf_size);
        region_count++;
        regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
        regions[region_count - 1].start_byte = cur_start;
        regions[region_count - 1].end_byte = cur_end;
...
// hex to decimal
/*int hex_to_dec(char *hexStr) {
printf("entering into devil\n");
printf("string %s\n",hexStr);
...
printf("length of string %d \n",len);
	...
	for (int i=len-1; i >=0; i-- ) {
		if((hexStr[i] >= '0') && (hexStr[i] <= '9')) {
			dec_val += (hexStr[i] - 48) * base;
			base = base * 16;
		}
		else if ((hexStr[i] >= 'A') && (hexStr[i] <= 'F')) {
			dec_val += (hexStr[i] - 55) * base;
			base = base * 16;
		}


	}
printf("end of the function and decimal value is %d\n",dec_val);
	return dec_val;
...
{
	printf("entered rtsp response func\n");
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
  ...
  while (byte_count < buf_size) {
	...
	printf("Memory %hhx: \n", mem[9]);
        memcpy(temp, &mem[9], 4);
	printf("temp before: %hhx, %hhx, %hhx, %hhx \n",temp[0], temp[1], temp[2], temp[3]);
        temp[3] = 0x0;
	printf("temp after: %hhx, %hhx, %hhx, %hhx \n",temp[0], temp[1], temp[2], temp[3]);
	...
	printf("message code %d\n", message_code);
        ...
        state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
        state_sequence[state_count - 1] = message_code;
	//printf("sttae seq %d: %d\n", state_count-1, state_sequence[state_count - 1]);
        mem_count = 0;
      } else {
        mem_count = 0;
      }
  ...
  if (mem) ck_free(mem);
  *state_count_ref = state_count;
	printf("exiting rtsp response func\n");
  return state_sequence;
}
