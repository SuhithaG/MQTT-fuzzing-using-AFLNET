...
u8* choose_source_region(u32 *out_len) {
  ...
  *out_len = 0;
  ...
  if (q->region_count) {
    ...
    if (len <= MAX_FILE) {
	//printf("ck alloc check in func1 before: %d\n", len);
      out = (u8 *)ck_alloc(len);
	//printf("ck alloc check in func1 after: %d\n", len);
...
u32 update_scores_and_select_next_state(u8 mode) {
  ...
  u32 *state_scores = NULL;
	//printf("ck alloc check in func2 before: %d\n", state_ids_count);
  state_scores = (u32 *)ck_alloc(state_ids_count * sizeof(u32));
	//printf("ck alloc check in func2 after: %d\n", state_ids_count);
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  unsigned int state_count;
	...
	//printf("response_buf_size %u\n", response_buf_size);
  if (!response_buf_size) return;
	//printf("check response codes fun call\n");
  unsigned int *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
	...
	//printf("ck alloc check in func3 before: \n");
  ...
  //Annotate the regions
	//printf("reached update_region_annotations\n");
...
{
//printf("entry into send over network function\n");
  int n;
  u8 likely_buggy = 0;
...
//printf("server address %u port number %u \n",serv_addr.sin_addr.s_addr,serv_addr.sin_port);
  ...
  if(connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    ...
    if (n== 1000) {
//	printf("returning without receiving packets on socket id %d \n",sockfd);
      close(sockfd);

      return 1;
    }
  }
//printf("generated socket %d\n",sockfd);
...
//printf("after receiving packets from server\n");
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  if (corpus_read_or_sync) {
	...
	//printf("ck alloc check in func4 before: %d\n", len);
    buf = (unsigned char *)ck_alloc(len);
	//printf("ck alloc check in func4 after: %d\n", len);
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (!q->trace_mini) {
		//printf("ck alloc check in func5 before: %d\n", MAP_SIZE >>3);
         q->trace_mini = ck_alloc(MAP_SIZE >> 3);
	//printf("ck alloc check in func5 after: %d\n", MAP_SIZE >> 3);
...
static void load_extras(u8* dir) {
  ...
  while ((de = readdir(d))) {
	...
	//printf("ck alloc check in func6 before: %ld\n", st.st_size);
    extras[extras_cnt].data = ck_alloc(st.st_size);
	//printf("ck alloc check in func6 after: %ld\n", st.st_size);
...
//printf("fork server pid %d \n",forksrv_pid);
//printf("st pipe zero %d \n",st_pipe[0]);
//printf("control pipe one %d \n",ctl_pipe[1]);
  if (!forksrv_pid) {
//printf("entry into the fork server child\n");
...
//printf("after exiting if condition \n");
  close(ctl_pipe[0]);
  close(st_pipe[1]);
...
static u8 run_target(char** argv, u32 timeout) {
//printf("entering into run target function \n");
...
//printf("target path in run target , %u \n",*target_path);
      execv(target_path, argv);
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
	...
	//printf("state aware mode %u\n", state_aware_mode);

    fd = open(q->fname, O_RDONLY);
    if (fd < 0) PFATAL("Unable to open '%s'", q->fname);

		//printf("ck alloc check in func7 before: %d\n", q->len);
    use_mem = ck_alloc_nozero(q->len);
	//printf("ck alloc check in func7 after: %d\n", q->len);
    ...
    if (state_aware_mode) {
	//printf("reached update state aware var\n");
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  for (i = 0; i < region_count; i++) {
	...
	//printf("func8: in if i: %d, region_count: %d, start byte: %d, end byte: %d\n", i, region_count, regions[i].start_byte, regions[region_count - 1].end_byte);
      len = regions[region_count - 1].end_byte - regions[i].start_byte + 1;
    } else {
	//printf("func8 in else i:%d, start byte: %d, end byte: %d\n", i, regions[i].start_byte, regions[i].end_byte);
      len = regions[i].end_byte - regions[i].start_byte + 1;
    }

    //Create a new message
    message_t *m = (message_t *) ck_alloc(sizeof(message_t));
	//printf("ck alloc check in func8 before: %d\n", len);
    m->mdata = (char *) ck_alloc(len);
	//printf("ck alloc check in func8 after: %d\n", len);
    m->msize = len;
...
AFLNET_REGIONS_SELECTION:;
  ...
  orig_in = in_buf;
	//printf("ck alloc check in func9 before: %u\n", in_buf_size);
  out_buf = ck_alloc_nozero(in_buf_size);
	//printf("ck alloc check in func9 after: %u\n", in_buf_size);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  /* Initialize effector map for the next step (see comments below). Always
	...
	//printf("ck alloc check in func10 before: %u\n", EFF_ALEN(len));
  eff_map    = ck_alloc(EFF_ALEN(len));
//printf("ck alloc check in func10 after: %u\n", EFF_ALEN(len));
  eff_map[0] = 1;
...
skip_interest:
  ...
  orig_hit_cnt = new_hit_cnt;
//printf("ck alloc check in func11 before: %u\n", len + MAX_DICT_FILE);
  ex_tmp = ck_alloc(len + MAX_DICT_FILE);
//printf("ck alloc check in func11 after: %u\n", len + MAX_DICT_FILE);
...
EXP_ST void check_binary(u8* fname) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
	...
	//printf("ck alloc ckeck in fun14 before : %ld\n", delim - env_path + 1);
        cur_elem = ck_alloc(delim - env_path + 1);
	//printf("ck alloc ckeck in fun14 after : %ld\n", delim - env_path + 1);
...
#endif /* ^!__APPLE__ */
//printf("checking memmem inputs. f_data : %s , f_len: %d, SHM_ENV_VAR: %s ,length: %d \n ",*(char*)f_data,f_len,*(char*)SHM_ENV_VAR, strlen(SHM_ENV_VAR) + 1);
...
static void save_cmdline(u32 argc, char** argv) {
  ...
  for (i = 0; i < argc; i++)
	...
	//printf("ck alloc ckeck in fun15 before : %d\n", len);
  buf = orig_cmdline = ck_alloc(len);
	//printf("ck alloc ckeck in fun15 after : %d\n", len);
...
//printf("step3\n");
  show_init_stats();
//printf("step4\n");	
//printf("after init status func\n");

  seek_to = find_start_position();
//printf("step5\n");
//printf("after find start position \n");
